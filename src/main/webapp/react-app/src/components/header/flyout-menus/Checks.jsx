...
import {checksCommandMap} from "@/constants/ChecksCommandMap.js";

export default function ChecksMenu() {

    const checksCommands = Array.from(checksCommandMap.entries()).filter(
        (item) =>
        ...
        <Menu as="div" className="relative ml-3">
            ...
            >
                <Menu.Items
                    ...
                    {checksCommands.map((item) => (
                        <Menu.Item key={item[0].toLowerCase()}>
                            {({active}) => (
                                <NavLink
                                    to={"/checks/" + item[0].toLowerCase()}
