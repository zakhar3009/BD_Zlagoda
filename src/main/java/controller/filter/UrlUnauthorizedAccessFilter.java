...
public class UrlUnauthorizedAccessFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
        ...
        HashMap<String, String> headers = CommandFactory.getAttributes(httpRequest, HashMap.class);
        if (headers != null && headers.get("command_name").equals("POST_LOGIN")) {
            filterChain.doFilter(servletRequest, servletResponse);
            return;
        } else if (!isUserRegistered(user) || !isUserAuthorizedForResource(httpRequest, user)) {
